...
export default {async fetch(request,env){
 ...
 try{
  ...
  if(path==='/reviews/batch'&&request.method==='POST'){
   ...
   if(c.reports.some(p=>!rows.some(r=>r.report_id===p.id)&&!existing.some(r=>r.report_id===p.id&&r.reviewer===first.reviewer)))throw new HttpError(400,'Decide every report before submitting this case.');
