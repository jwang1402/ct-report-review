...
export function validateReview(r){if(!r||r.schema!=='ct-review-v1'||!Number.isSafeInteger(r.release_id)||r.release_id<=0||!['ACCEPT','PARTIAL_ACCEPT','REJECT'].includes(r.decision)||!['case_id','report_id','model_name','reviewer','comment','submission_id'].every(k=>typeof r[k]==='string')||!r.reviewer.trim()||r.reviewer.trim().length>100||r.comment.length>10000||!r.model_name.trim()||r.model_name.length>200||!/^[-\w]{1,100}$/.test(r.case_id)||!/^[-\w]{1,100}$/.test(r.report_id)||!/^[-\w]{16,100}$/.test(r.submission_id)||r.decision==='PARTIAL_ACCEPT'&&!r.comment.trim())throw new HttpError(400,'Enter your name, a decision, and a comment for partial acceptance.');return {...r,reviewer:r.reviewer.trim()};}
...
export default {async fetch(request,env){
 ...
 try{
  ...
  if(path==='/reviews/batch'&&request.method==='POST'){
   ...
   if(new Set(rows.map(r=>r.submission_id)).size!==rows.length||new Set(rows.map(r=>r.report_id)).size!==rows.length||rows.some(r=>r.case_id!==first.case_id||r.release_id!==first.release_id||!c.reports.some(p=>p.id===r.report_id&&p.model_name===r.model_name)))throw new HttpError(400,'Invalid report selection');
   ...
   if(c.reports.some(p=>!rows.some(r=>r.report_id===p.id)&&!existing.some(r=>r.report_id===p.id)))throw new HttpError(400,'Decide every report before submitting this case.');
  ...
  if(path==='/reviews'&&request.method==='POST'){
   ...
   if(c.reports.length>1)throw new HttpError(400,'Use case submission after deciding every report.');
