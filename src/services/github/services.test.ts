...
import {parseReviewIssue,buildReviewIssueUrl,reviewsCSV,counts} from './issues';
import {guardSize} from './releaseAssets';
import type {Review} from '../../types';
const review:Review={schema:'ct-review-v1',case_id:'CT001',report_id:'model-a',model_name:'Example model',decision:'3',comment:'Location is right.\nSize needs review.',submission_id:'unique-submission',release_id:42};
describe('GitHub review interchange',()=>{
 ...
 it('allows optional feedback for ratings',()=>expect(()=>buildReviewIssueUrl({...review,comment:''})).not.toThrow());
...
it('keeps legacy decisions separate from numeric rating counts and exports',()=>{const base={...review,reviewer:'Doctor',github_issue_number:1,created_at:'date',url:''};const rows=[{...base,decision:'5' as const},{...base,decision:'ACCEPT' as const}];expect(counts(rows)).toMatchObject({'5':1,'1':0,ACCEPT:1});const csv=reviewsCSV(rows,new Map());expect(csv).toContain('"rating","legacy_decision"');expect(csv).toContain('"5",""');expect(csv).toContain('"","ACCEPT"');});
