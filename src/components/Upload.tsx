...
export function Upload(){
 ...
 async function prepare(){setError('');setBusy(true);setStage('Validating imaging data…');try{
  ...
  if(JSON.stringify(meta).length>2_000_000)throw new Error('Case metadata is too large. Shorten report text.');
 ...
 const releaseUrl=prepared?`${repositoryUrl}/releases/new?${new URLSearchParams({tag:prepared.tag,title:`${prepared.meta.case_id} - ${prepared.meta.case_name}`,body:'CT Report Review case. Upload case.json and the original imaging asset before publishing.'})}`:'';
