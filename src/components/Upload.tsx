...
export function Upload(){
 const [destination,setDestination]=useState<'drive'|'github'>('drive');
 ...
 async function prepare(){setError('');setBusy(true);setStage('Validating imaging data…');try{
  ...
  setPrepared({meta,blob,tag:`case-${id}-${Date.now()}`});setStage('Case prepared');
 ...
 return <main className="page upload-page"><div className="page-heading"><div><span className="eyebrow">RESEARCH WORKSPACE / PUBLISH</span><h1>Upload a case</h1><p>Prepare your CT and reports, then upload to your selected Google Drive folder.</p></div><select aria-label="Upload destination" value={destination} disabled={busy} onChange={e=>setDestination(e.target.value as 'drive'|'github')}><option value="drive">Google Drive</option><option value="github">GitHub Releases (legacy test)</option></select></div>
 ...
 <aside>{destination==='drive'&&<DriveUpload prepared={prepared} onBusy={setBusy}/>}<div className="form-card sticky"><h2>{destination==='drive'?'Prepare case files':'Publish to your repository'}</h2><p className="muted">{destination==='drive'?'Validate your CT and reports here, then upload using the Google Drive panel above.':'Prepare the files here, then upload both assets on GitHub.'}</p>{destination==='github'&&<ol className="steps-list"><li>Download case.json and the CT asset.</li><li>Open the prefilled GitHub Release.</li><li>Attach both files and publish.</li><li>Wait for Pages sync, then refresh Clinical Review.</li></ol>}<label className="checkbox-label"><input type="checkbox" checked={consent} onChange={e=>{setConsent(e.target.checked);setPrepared(null);}}/>This case contains only public, synthetic, or appropriately de-identified data.</label><button className="button primary full" disabled={busy} onClick={prepare}>{busy?'Preparing…':'Prepare Case'}<ArrowUpRight size={16}/></button>{stage&&<p className="status-line" role="status">{busy?<span className="spinner"/>:<Check size={15}/>} {stage}</p>}{error&&<div role="alert" className="error">{error}</div>}
 {prepared&&destination==='github'&&<div className="prepared"><button className="button secondary full" onClick={()=>saveFile(JSON.stringify(prepared.meta,null,2),'case.json','application/json')}><Download size={16}/>Download case.json</button><button className="button secondary full" onClick={()=>saveFile(prepared.blob,prepared.meta.imaging.filename)}><Download size={16}/>Download CT asset</button><a className="button primary full" href={releaseUrl} target="_blank" rel="noreferrer">Open GitHub Release<ArrowUpRight size={16}/></a><small>Attach <b>case.json</b> and <b>{prepared.meta.imaging.filename}</b>. After publishing, wait for the Pages synchronization workflow.</small></div>}
 {destination==='github'&&<div className="note">Original CT data stays in Releases. A temporary copy is synchronized to Pages for browser viewing. Test mirror limit: 800 MB total.</div>}</div></aside></div></main>;
}
